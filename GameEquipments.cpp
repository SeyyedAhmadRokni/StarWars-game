...
Arrow::Arrow(Point p, string imageAddress, int damage) : Box(p, ARROW_WIDHT, ARROW_HEIGHT, imageAddress, ARROW_RATE)
{
...
{
    if (x <= windowWidth && x >= 0 && y <= windowHeight && windowHeight >= 0)
    {
        return true;
    }
    return false;
...
SpeedItem::SpeedItem(Point position) : Item(position.x, position.y, SPEED_ITEM_EXISTENCE_TIME,
...
GaurdItem::GaurdItem(Point position) : Item(position.x, position.y, GAURD_ITEM_EXISTENCE_TIME,
