...
void EnemyManager::shoot(int numberOfColumns)
{
    if (!enemyShootTimer->isTimeToShot())
    {
        return;
    }
    set<int> safeColumns = enemyShootTimer->getSafeColumns();

    for (int i = 1; i <= numberOfColumns; i++)
    {
        if (safeColumns.count(i))
        {
            continue;
        }
        for (int j = 0; j < enemies.size(); j++)
        {
            if (isInColumn(enemies[j], i) &&
