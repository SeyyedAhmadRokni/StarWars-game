...
bool ItemManager::isChanceWithMakingItem()
{
    ...
    int identifier = rand() % totalChances;
                                         ...
                                         vector<Enemy *> &enemies, Window *window)
{
    for (int i = 0; i < enemies.size(); i++)
    {
        ...
        {
            if (enemies[i]->hasCollision(players[j]) && !players[j]->hasGaurd())
            {
                delete enemies[i];
                enemies.erase(enemies.begin() + i);
...
set<int> EnemyShootTimer::chooseColumnsEasy()
{
    ...
    int emptyColumnsDeterminative = rand() % totalChances;
...
EnemyShootTimer::EnemyShootTimer(int col, DifficultyLevel lvl)
{
    numberOfColumns = col;
    this->level = lvl;
    time(&lastShot);
...
set<int> EnemyShootTimer::getSafeColumns()
{
    time(&lastShot);
